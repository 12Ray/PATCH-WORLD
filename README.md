...
| 무기 | 체력 | 고유 능력 | 강점 | 약점 |
| --- | ---: | --- | --- | --- |
| 칼 · Sword | 5 | `K` 좌우 대시, 대시 중 무적, 쿨타임 5초 | 짧은 무적과 높은 기동성 | 대시가 Motion Tax 열을 높임 |
| 주먹 · Gauntlet | 7 | 기본 더블 점프 + `K` 최대 10초 충전 강타 | 높은 생존력, 공중 복귀와 충전 범위 공격 | 이동 속도 5% 및 공격 속도 감소 |
| 총 · Gun | 3 | `K`를 누르는 동안 최대 5초 전방 레이저 | 긴 사거리, 지속 화력과 0.32초 공격 주기 | 낮은 체력 |
