...
현재 캠페인을 기존 탑다운 아레나에서 **횡스크롤 플랫포머 로그라이크**로 전환하고 있습니다. 첫 번째 구현 단계로 ROOM 1 `DAMAGE LAB`에 달리기, 점프, 플랫폼 충돌과 낙하 복귀가 적용되었습니다.

## 현재 개발 상태

| 영역 | 상태 | 설명 |
|---|---|---|
| ROOM 1 — Damage Lab | 플랫포머 수직 슬라이스 구현 | 가속 이동, 중력, 가변 점프, 코요테 타임, 점프 버퍼, 플랫폼과 데이터 피트 |
| ROOM 1 전투 | 전환 중 | 기존 crawler 2종과 패치 선택 흐름 유지. 신규 적 5종은 다음 구현 단계 |
| ROOM 2 — Temporal Hall | 기존 버전 유지 | 탑다운 룸과 시간 규칙이 동작하며 이후 플랫포머로 전환 예정 |
| ROOM 3 — Collision Archive | 기존 버전 유지 | 탑다운 충돌 퍼즐이 동작하며 이후 플랫포머로 전환 예정 |
| THE OPTIMIZER | 기존 버전 유지 | 캠페인 보스와 엔딩 흐름 동작 |
| PATCH//SURVIVE | 기존 버전 유지 | 무한 생존, 패치 성장, Fusion 시스템 동작. 캠페인 전환 완료 후 재설계 예정 |

플랫포머 전환은 ROOM 1부터 순차적으로 진행합니다. 생존 모드와 아직 전환되지 않은 룸에는 기존 8방향 이동을 유지해 현재 플레이 가능한 흐름을 보존합니다.
...
점프는 버튼을 짧게 누르면 낮아지고 길게 누르면 높아집니다. 발판에서 벗어난 직후에도 약 0.10초 동안 점프할 수 있으며, 착지 직전 입력한 점프도 약 0.12초 동안 보존됩니다.

### 기존 룸과 생존 모드

아직 플랫포머로 전환되지 않은 영역에서는 `WASD` 또는 방향키로 8방향 이동합니다. 공격과 상호작용 키는 동일합니다.
...
1. ROOM 1 최종 플레이어 및 적 스프라이트 스트립 제작
2. Patch Mite, Checksum Hopper, Pulse Turret, Repair Leech 구현
3. 중간보스 Overflow Warden과 ROOM 1 완료 루프 구현
4. 검증된 파이프라인으로 Temporal Hall과 Collision Archive 전환
5. 세 캠페인 룸 완성 후 PATCH//SURVIVE를 side-view 체계로 재설계
